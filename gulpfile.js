...
//co处理
gulp.task('build-co', gulp.series('cleanCo', 'co-native', 'co-scripts', 'co-dom', 'co-libs','co-debug', 'co-ui', 'co-css', 'co-img', 'co-font'));
...
gulp.task('dist-js', gulp.series('dist-libs', 'dist-ui', 'dist-scripts', 'dist-dom','dist-native', 'dist-debug'));
...
gulp.task('open', function(cb) {
    gulp.src(paths.examples.index).pipe(open('', {
        url: 'http://localhost:3000/' + paths.examples.index
    }));
    cb();
});
